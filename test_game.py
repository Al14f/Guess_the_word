...
# ============== VALUTA_TENTATIVO_GUI ==============
@pytest.mark.parametrize(
    "tentativo,parola,idx_check,colore",
    [
        ("HELLO", "HELLO", 0, GREEN),
...
# ============== CALCOLA_RIGHE ==============
@pytest.mark.parametrize(
    "stato,tentativi_count,expect_max_positive",
    [
        ("GIOCO", 1, False),
        ("GIOCO", 10, True),
        ("FINE", 10, True),
...
def test_gestisci_scroll(y, expected_row):
    g = {
        "view_start_row": 2,
        "tentativi_fatti": [("T", [GREEN])] * 10,
    }
    e = Mock(y=y)
    gestisci_scroll(e, g)
...
def test_conferma_parola_vittoria():
    g = {
        "current_guess": "HELLO",
        "parola_segreta": "HELLO",
        "tentativi_fatti": [],
    ...
    conferma_parola(g)
    assert g["stato"] == "FINE" and g["messaggio_fine"] == "COMPLIMENTI!"


def test_conferma_parola_sconfitta():
    g = {
        "current_guess": "X",
        "parola_segreta": "Z",
        "tentativi_fatti": [("T", [GRAY])] * 19,
        "limite": 20,
        "modalita": "tentativi",
        "view_start_row": 0,
        "stato": "GIOCO",
    ...
    conferma_parola(g)
    assert g["stato"] == "FINE" and "PAROLA" in g["messaggio_fine"]
